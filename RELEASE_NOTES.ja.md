...
- 公開前に製品名をAGI Keysへ変更し、プラグインと全アクションのUUID名前空間を `com.dualform.agikeys` へ移しました。
- 操作一覧、設定、表示フィードバック、資料の英語対応。日本語も維持します。
...
公開前の名称変更に伴い、プラグインUUIDを `com.dualform.agikeys` へ変更しました。旧開発UUIDを参照する事前検証用プロファイルでは、AGI Keysの操作を再配置してください。
...
- AGI Keys 0.1.0.66のソースは723件の自動検査、型検査、Stream Deck公式パッケージ検査に合格しています。全67操作の実機試験、全ショートカット、再起動後の復帰を保証する結果ではありません。
