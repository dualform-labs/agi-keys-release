...
import {
  ...
  reasoningDialValue,
  truncateFeedback,
} from "./controller-display.js";

export { reasoningDialValue } from "./controller-display.js";
  ...
  private refreshUsageInBackground(): void {
    ...
    const pending = this.microBridge.refreshUsage().then(async (snapshot) => {
      ...
      await Promise.all([
        ...
        ...[...this.usageLimitActions.values()].map((registration) => this.renderSafely(
          "E_RENDER_USAGE_LIMIT",
          registration.action.id,
          () => this.renderUsageLimit(registration),
        )),
        ...[...this.usageOverviewActions.values()].map((action) => this.renderSafely(
          "E_RENDER_USAGE_OVERVIEW",
          action.id,
          () => this.renderUsageOverview(action),
        )),
      ]);
    }).catch(() => {
